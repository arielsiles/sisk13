 ...
 * @version 3.0
 */
@Name("contributionStatusReportAction")
@Scope(ScopeType.PAGE)
public class ContributionStatusReportAction extends GenericReportAction {
    ...
    protected String getEjbql() {
        ...
        */
        ejbql = " SELECT " +
                " voucherDetail.account AS accountTypeName, " +
                " partner.productiveZone.number || ' ' || partner.productiveZone.name AS gab, " +
                " partner.idNumber, " +
                " partner.idNumber AS code, " +
                " '' AS accountTypeCurrency, " +
                " partner.firstName || ' ' || partner.lastName || ' ' || partner.maidenName AS partnerName, " +
                " (SUM(voucherDetail.credit) - SUM(voucherDetail.debit)) AS balanceMN, " +
                " (SUM(voucherDetail.creditMe) - SUM(voucherDetail.debitMe)) AS balanceME " +
                " FROM VoucherDetail voucherDetail" +
                " JOIN voucherDetail.voucher voucher" +
                " JOIN voucherDetail.partner partner" +
                " WHERE voucher.state <> 'ANL'" +
                " AND voucherDetail.partner is not null" +
                " AND voucher.date <= #{contributionStatusReportAction.endDate} " +
                " AND voucherDetail.account = '3110100000'" +
                " GROUP BY partner.productiveZone.number, partner.firstName, partner.lastName, partner.maidenName" +
                " ORDER BY partner.productiveZone.number, partner.firstName, partner.lastName, partner.maidenName";
    ...
    public void generateReport() {
        ...
        String documentTitle = messages.get("Reports.account.contributionStatus.title");
        ...
        super.generateReport(
                ...
                "/customers/reports/contributionStatusReport.jrxml",
                PageFormat.LETTER,
