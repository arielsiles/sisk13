...
- Upload via `<s:fileUpload data="#{dispatchRouteAction.mapFile.value}" ...>` hacia un holder `File mapFile` en el action (mismo patron que la foto de Activos Fijos / Dismissal): el componente NUNCA escribe en la entidad, por lo que guardar/aprobar sin re-subir jamas pone la imagen en null. Si `mapFile.value` trae bytes (archivo nuevo), `create()`/`update()`/`approve()` los redimensionan y reemplazan `mapImage`; si no, la imagen queda intacta. El preview se sirve inline como data-URI (`getMapImageDataUri()`), no via el resource servlet de Seam.
- Navegacion (igual que el Vale de Despacho): `create()`->`SUCCESS`->redirect a la MISMA pantalla (la conversacion se propaga, `op=UPDATE` queda en modo edicion); `update()`->`REDISPLAY` (se queda); `approve()`/`inactivate()`/`delete()`/`cancel()`->listado. NO se usa `<end-conversation/>` en las reglas de la pagina de edicion (rompe el redirect de los `a4j` de los modales Borrar/Inactivar); la cierra el `<end-conversation/>` de la pagina de listado.
