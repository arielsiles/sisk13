...
- Upload via `<s:fileUpload data="#{dispatchRoute.mapImage}" fileName="#{dispatchRouteAction.uploadedMapName}">` (bind directo a la entidad, mismo patron que la foto de Activos Fijos; el buffer en el action NO se poblaba en el primer `create()`). `uploadedMapName` solo senala que se adjunto archivo nuevo; el action redimensiona los bytes in-place en `create()`/`update()`/`approve()` antes de persistir. El preview se sirve inline como data-URI (`getMapImageDataUri()`), no via el resource servlet de Seam.
